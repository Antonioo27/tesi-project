// file: src/main/java/ghs/analyzer/graph/ChaCallGraphAnalyzer.java
package ghs.analyzer.graph;

import ghs.analyzer.heuristics.FocalClassHeuristic;
import ghs.analyzer.heuristics.FocalMethodHeuristic;
import ghs.analyzer.heuristics.TestClassifier;
import ghs.analyzer.model.CgStats;
 ...
 *
 * Passi:
 *  1) BFS dal metodo di test (limiti: maxDepth, maxVisited, prune libs).
 *  2) Classificazione UNIT/INTEGRATION (TestClassifier).
 *  3) Se UNIT → stima focal class e focal method (Unique Direct Call → fallback euristico).
...
public final class ChaCallGraphAnalyzer implements CallGraphAnalyzer {
  ...
  public TestRecord analyzeOne(
    ...
    // copie "final" per l'uso in lambda
    final Map<MethodSignature, Integer> dist = distance;

    // Metodi di classi DI PROGETTO raggiunti
    List<MethodSignature> projectTargets = dist
      .keySet()
      .stream()
      .filter(ms ->
    ...
    if (cr.kind() == TestKind.UNIT) {
      // 3.a) FOCAL CLASS
      String candidateByName = classHeu.guessFocalClassFromTestName(testClass); // es. ...FooTest -> ...Foo
      String pkg = testClass.contains(".")
        ? testClass.substring(0, testClass.lastIndexOf('.'))
        : "";
      String base = simpleName.apply(candidateByName);
      final String baseName = base; // effectively-final per i lambda
      ...
      Optional<MethodSignature> byName = projectTargets
        .stream()
        .filter(ms ->
          ms.getDeclClassType().getFullyQualifiedName().endsWith("." + baseName)
        )
      ...
      Optional<String> byMinDistanceClass = projectTargets
        ...
        .stream()
        .sorted(
          Comparator.comparingInt(e -> e.getValue().orElse(Integer.MAX_VALUE))
        )
      ...
      // >>> QUI: variabile final per uso nei lambda
      ...
      // 3.b) FOCAL METHOD
      final Set<MethodSignature> directCallsFromTest = cg
        .callsFrom(testSig)
        .stream()
        .map(call -> call.getTargetMethodSignature())
        .collect(Collectors.toSet());

      List<MethodSignature> directCallsToFocal = focalClassMethods
        .stream()
      ...
      if (directCallsToFocal.size() == 1) {
        ...
        focalMethodSig = methodHeu.selectFocalMethod(
          focalClassFqnFinal,
          focalClassMethods
        );
      }
    ...
    double score = scorer.score(
      new UnitIntegrationScorer.Features(
        ...
        stats.projectCalls(), // numero invocazioni verso classi progetto
